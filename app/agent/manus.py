...
class Manus(BrowserAgent):
    ...
    max_steps: int = 8  # Reduced from 20 to 8 for faster completion

    # Initialize LLM with local model settings
    llm: LLM

    # Add general-purpose tools to the tool collection - FIXED: Pass LLM to BrowserUseTool
    def __init__(self, **data):
        super().__init__(**data)
        # Initialize tools with LLM instance
        self.available_tools = ToolCollection(
            PythonExecute(), 
            BrowserUseTool(llm=self.llm),  # Pass LLM instance to BrowserUseTool
            StrReplaceEditor(), 
            Terminate()
        )
